            ...
            steps {
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-key']]) {
                        ...
                        def frontend_service_url = bat(script: 'powershell -Command "kubectl get service frontend-service -o jsonpath=\'{.status.loadBalancer.ingress[0].hostname}\'"', returnStdout: true).trim()
